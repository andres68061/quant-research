...
def fetch_banxico_series(
    ...
    if not api_key:
        raise ValueError("Banxico API key not found. Set BANXIC_API_KEY environment variable.")
    ...
    try:
        ...
        raise ValueError(f"Banxico API request failed: {e}") from e
...
def get_cetes28_returns(
    start_date: str, end_date: str, api_key: Optional[str] = None, exact28: bool = False
) -> pd.Series:
    """
...
def get_usdmxn_rate(start_date: str, end_date: str, api_key: Optional[str] = None) -> pd.Series:
    """
...
def cache_cetes28_data(
    start_date: str = "2019-01-01", output_path: Optional[Path] = None
) -> pd.Series:
    """
