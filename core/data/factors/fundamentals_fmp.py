...
def fetch_bulk_ratios_year(
    year: int, period: str = "quarter", refresh_cache: bool = False
...
def load_bulk_ratios_range(
    start_year: int, end_year: int, period: str = "quarter", refresh_cache: bool = False
) -> pd.DataFrame:
    ...
    for src, dst in col_map.items():
        out[dst] = (
            pd.to_numeric(all_df.get(src), errors="coerce") if src in all_df.columns else pd.NA
        )
...
def fetch_fundamentals_universe(
    symbols: List[str], period: str = "quarter", limit: int = 64
) -> pd.DataFrame:
    frames = []
    for sym in symbols:
...
def dailyize_fundamentals(
    fund_df: pd.DataFrame, start: Optional[pd.Timestamp] = None, end: Optional[pd.Timestamp] = None
) -> pd.DataFrame:
    ...
    for sym, g in fund_df.groupby("symbol"):
        ...
        ts = g.set_index("asof_date")[
            ["pe", "pb", "ps", "roe", "roa", "gross_margin", "debt_to_equity", "revenue_growth"]
        ]
