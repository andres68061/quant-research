...
- ``scripts/audit_data_health.py`` — writes the JSON snapshot + refreshes
...
from core.data.validation import validate_price_panel, violations_as_dicts
...
# Per-symbol datasets fetched by scripts/fetch_fmp_datasets.py.
...
def audit_panel_invariants() -> dict[str, Any]:
    """
    Structural checks on the canonical price panel (core.data.validation).
    ...
    flaws.append(
        {
            ...
            "detail": (
                ...
                "core.data.fmp.intraday.load_intraday_bars, which detects and repairs exactly "
