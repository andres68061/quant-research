...
class MarketCapCalculator:
    ...
    def load_prices(self) -> pd.DataFrame:
        ...
        if self._prices is None:
            if self.prices_file.exists():
                ...
                logger.info(
                    f"Loaded prices: {len(self._prices)} dates, {len(self._prices.columns)} symbols"
                )
            else:
    ...
    def fetch_all_shares_outstanding(
        self, tickers: list, delay: float = 0.5, resume: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if resume and self.shares_file.exists():
            ...
            logger.info(
                f"Resuming: {len(already_fetched)} already fetched, {len(tickers)} remaining"
            )
    ...
    def calculate_market_caps(
        self, tickers: Optional[list] = None, save: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if shares_df.empty:
            raise ValueError(
                "No shares outstanding data available. Run fetch_all_shares_outstanding first."
            )
        ...
        available_tickers = [
            t for t in tickers if t in prices.columns and t in shares_df["ticker"].values
        ]
        ...
        print(
            f"  Date range: {result_df.index.get_level_values('date').min().date()} to {result_df.index.get_level_values('date').max().date()}"
        )
    ...
    def get_market_cap_on_date(
        self, date: pd.Timestamp, tickers: Optional[list] = None
    ) -> pd.Series:
        """
    ...
    def get_weights_on_date(self, date: pd.Timestamp, tickers: Optional[list] = None) -> pd.Series:
        """
    ...
    def get_summary_stats(self) -> Dict:
        ...
        if market_caps_df is not None and not market_caps_df.empty:
            stats["tickers_with_market_caps"] = len(
                market_caps_df.index.get_level_values("ticker").unique()
            )
