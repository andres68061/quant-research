...
def _strip_tz(ts: pd.Timestamp) -> pd.Timestamp:
    """Remove timezone info so the timestamp can be compared to tz-naive indices."""
...
def resolve_sp500_historical_csv(data_dir: Path = Path("data")) -> Path:
    ...
    Filenames from upstream may include a snapshot date in parentheses; this function
    picks the file with the latest modification time among matches so renames do not
    break callers.
    ...
    matches = sorted(
        data_dir.glob(_HISTORICAL_CSV_GLOB),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    if not matches:
        raise ConfigError(
            f"No file matching {_HISTORICAL_CSV_GLOB!r} under {data_dir.resolve()}. "
            "Place the historical constituents CSV in that folder, or run the snp500 "
            "updater that copies from the fja05680/sp500 repository."
        )
    chosen = matches[0]
    if len(matches) > 1:
        logger.info(
            "Using newest of %d files matching %s: %s",
            len(matches),
            _HISTORICAL_CSV_GLOB,
            chosen.name,
        )
    return chosen
...
class SP500Constituents:
    ...
    def __init__(self, csv_path: Optional[Path] = None, data_dir: Optional[Path] = None):
        """
        Initialize S&P 500 constituents manager.

        Args:
            csv_path: Path to historical constituents CSV. If None, resolves via
                :func:`resolve_sp500_historical_csv` under ``data_dir`` or ``data/``.
            data_dir: When ``csv_path`` is None, directory to search for the newest
                file matching ``S&P 500 Historical Components & Changes*.csv``.
    ...
    def get_constituents_on_date(self, date: pd.Timestamp) -> List[str]:
        ...
        date = _strip_tz(pd.Timestamp(date))
    ...
    def get_constituents_series(
        ...
        if start_date is not None:
            df = df[df.index >= _strip_tz(pd.Timestamp(start_date))]
        if end_date is not None:
            df = df[df.index <= _strip_tz(pd.Timestamp(end_date))]
    ...
    def get_ticker_history(self, ticker: str) -> pd.DataFrame:
        ...
        for date, row in self._constituents.iterrows():
            history.append({"date": date, "in_sp500": ticker in row["tickers"]})

        df = pd.DataFrame(history)
        df["date"] = pd.to_datetime(df["date"])
        return df

    def filter_universe_by_date(self, tickers: List[str], date: pd.Timestamp) -> List[str]:
        """
