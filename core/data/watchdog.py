...
(:mod:`core.data.validation`, impossible values), and *guarded writes*
(:mod:`core.data.artifacts`, a build cannot clobber a panel). All three are
...
Severity contract matches :mod:`core.data.validation`:
...
ACKNOWLEDGED_INVARIANTS: dict[str, str] = {
    "extreme_return": (
        ...
        "rule); core.data.returns rejects them at compute time"
    ),
}
...
def check_panels_not_collapsed(baseline: dict[str, int]) -> list[Check]:
    ...
    done by something that did not go through :mod:`core.data.artifacts` at all —
...
def check_structural_invariants() -> list[Check]:
    """Run the panel invariants from :mod:`core.data.validation` on the price panel."""
    from core.data.validation import validate_price_panel
