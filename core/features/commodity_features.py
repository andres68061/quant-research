...
def create_ml_features(
    ...
    # ============================================================================
    df["downside_dev_expanding"] = calculate_downside_deviation_expanding(log_returns).shift(
        1
    )  # LAGGED
    ...
    # ============================================================================
    df["downside_dev_21d"] = calculate_downside_deviation_rolling(log_returns, 21).shift(
        1
    )  # LAGGED
    df["downside_dev_63d"] = calculate_downside_deviation_rolling(log_returns, 63).shift(
        1
    )  # LAGGED
...
def prepare_ml_dataset(
    ...
    for symbol in prices_df.columns:
        ...
        if len(price_series) < min_training_days + 5:  # +5 for test week
            logger.warning(
                f"Insufficient data for {symbol}: {len(price_series)} days (need {min_training_days + 5})"
            )
            continue
        ...
        if len(features_df) >= min_training_days:
            ...
            logger.info(
                f"Prepared ML dataset for {symbol}: {len(features_df)} rows, {len(features_df.columns)} features"
            )
...
def get_train_test_split_walk_forward(
    ...
    if total_rows < initial_train_days + test_days:
        logger.warning(
            f"Not enough data for walk-forward: {total_rows} rows (need {initial_train_days + test_days})"
        )
        return splits
...
def check_class_imbalance(y: pd.Series, threshold: float = 0.65) -> Dict:
    ...
    if is_imbalanced:
        result["recommendation"] = (
            f"Use class_weight='balanced' in model (imbalance > {threshold*100:.0f}%)"
        )
    else:
...
def check_outliers(returns: pd.Series, z_threshold: float = 3.0) -> Dict:
    ...
    if result["outlier_pct"] < 1.0:
        result["interpretation"] = (
            "Low outlier count (<1%) - likely real price movements. Keep all data."
        )
    elif result["outlier_pct"] < 3.0:
        result["interpretation"] = (
            "Moderate outliers (1-3%) - review dates for data errors vs real events."
        )
    else:
        result["interpretation"] = (
            "High outlier count (>3%) - may indicate data quality issues. Consider winsorizing."
        )
...
def create_ml_features_with_transparency(
    ...
    logger.info(
        f"Feature engineering complete for {symbol}: {metadata['final_rows']} rows, {metadata['total_features']} features"
    )
