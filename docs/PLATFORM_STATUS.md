...
- **Prioritized backlog**: see [docs/BACKLOG.txt](BACKLOG.txt) at the repo root.
- **Data & factor inventory** (artifacts, sources, academic gap map): [DATA_INVENTORY.md](data/DATA_INVENTORY.md).
...
| Data pipeline | In place | Parquet + DuckDB under `data/`; update scripts in `scripts/`; FF5 daily from Kenneth French library ([DATA_INVENTORY.md](data/DATA_INVENTORY.md)) |
...
| Event-driven backtest (v0 + HTTP) | In place | `core/backtest/events/` + `POST /backtest/events/simulate` ([EVENT_DRIVEN_BACKTEST.md](research/EVENT_DRIVEN_BACKTEST.md)); intraday/UI still open |
...
- **Event-driven backtesting** — v0 core and **simulate REST** are in; **intraday bars and UI** still open. Cross-sectional factor path remains primary for equity research. [EVENT_DRIVEN_BACKTEST.md](research/EVENT_DRIVEN_BACKTEST.md).
- **Data / factor transparency** — see [DATA_INVENTORY.md](data/DATA_INVENTORY.md) for Parquet artifacts, ingestion sources, and gap vs common systematic factor families.
...
Cron reference file: [`scripts/ops/crontab.txt`](../scripts/ops/crontab.txt). Restore with `crontab scripts/ops/crontab.txt`. Full schedule details in [DATA_INVENTORY.md §4](data/DATA_INVENTORY.md).
...
- Data inventory and factor coverage: [DATA_INVENTORY.md](data/DATA_INVENTORY.md)
- Event-driven backtest (design): [EVENT_DRIVEN_BACKTEST.md](research/EVENT_DRIVEN_BACKTEST.md)
