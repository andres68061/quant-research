...
- **[high] A single flat cost is applied regardless of liquidity** — Backtests charge one basis-point figure per trade for every name. That flatters any strategy that trades illiquid stocks — most of all the liquidity factors, which deliberately go long the hardest-to-trade names. Use the dollar-ADV cost schedule in core/data/liquidity.py before believing a liquidity result.
