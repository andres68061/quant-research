...
Usage:
    ...
    /opt/anaconda3/envs/quant/bin/python scripts/backfill_expanded_universe.py --steps prices,panels
    /opt/anaconda3/envs/quant/bin/python scripts/backfill_expanded_universe.py --skip universe

Leave it running with output captured:
    ... scripts/backfill_expanded_universe.py > logs/expanded_backfill.log 2>&1 &
"""
...
from core.data.fmp.prices import generate_date_chunks
from core.data.fmp.storage import load_fetch_windows
...
def step_commands(universe_file: Path) -> dict[str, list[list[str]]]:
    ...
    return {
        "universe": [[PYTHON, "scripts/build_fmp_universe.py"]],
        "prices": [[PYTHON, "scripts/fetch_fmp_prices.py", *universe_args]],
        "fundamentals": [[PYTHON, "scripts/fetch_fmp_fundamentals.py", *universe_args]],
        "market_caps": [[PYTHON, "scripts/fetch_fmp_market_caps.py", *universe_args]],
        "panels": [
            [PYTHON, "scripts/fetch_fmp_prices.py", "--build-panel"],
            [PYTHON, "scripts/build_ohlcv_panel.py"],
            [PYTHON, "scripts/build_fundamentals_panel.py"],
            [PYTHON, "scripts/build_event_factors.py"],
            [PYTHON, "scripts/build_dollar_adv.py"],
        ],
    }
