...
Usage:
    /opt/anaconda3/envs/quant/bin/python scripts/cutover_canonical_panel.py --dry-run
    /opt/anaconda3/envs/quant/bin/python scripts/cutover_canonical_panel.py
    /opt/anaconda3/envs/quant/bin/python scripts/cutover_canonical_panel.py --rebuild
"""
...
REBUILD_STEPS: tuple[tuple[str, list[str]], ...] = (
    ("index membership labels", [PYTHON, "scripts/build_index_membership.py"]),
    ...
    ("price factors", [PYTHON, "scripts/build_price_factors.py"]),
    ("OHLCV + microstructure", [PYTHON, "scripts/build_ohlcv_panel.py"]),
    ("PIT fundamentals + factors", [PYTHON, "scripts/build_fundamentals_panel.py"]),
    ("event + vendor factors", [PYTHON, "scripts/build_event_factors.py"]),
    ("dollar ADV", [PYTHON, "scripts/build_dollar_adv.py"]),
    ("data health audit", [PYTHON, "scripts/audit_data_health.py"]),
)
...
def main() -> None:
    ...
    if args.refresh_first:
        ...
        for label, command in (
            ...
            ("rebuild expanded panel", [PYTHON, "scripts/fetch_fmp_prices.py", "--build-panel"]),
        ):
