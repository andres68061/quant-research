...
#
# Usage: INGEST_WAVES="1" bash scripts/ingest/chain_after_current.sh
set -uo pipefail
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$REPO" || exit 1
WAVES="${INGEST_WAVES:-1}"

running() { pgrep -f "bin/python.*scripts/ingest/ingest_fmp\.py" | grep -v "^$$\$"; }
...
exec /opt/anaconda3/envs/quant/bin/python "$REPO/scripts/ingest/spawn_ingest_daemon.py" --waves "$WAVES" --force
