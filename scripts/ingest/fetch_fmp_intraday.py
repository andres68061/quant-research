...
``core.data.vendors.fmp.intraday.load_intraday_bars``, which detects and repairs splits
that occurred after the fetch (a blanket re-adjustment would double-adjust).

Usage:
    /opt/anaconda3/envs/quant/bin/python scripts/ingest/fetch_fmp_intraday.py \
        --interval 1hour --years 10
    /opt/anaconda3/envs/quant/bin/python scripts/ingest/fetch_fmp_intraday.py \
        --interval 1min --symbols AAPL,MSFT,SPY --years 5
    /opt/anaconda3/envs/quant/bin/python scripts/ingest/fetch_fmp_intraday.py --estimate-only
"""
...
from core.data.vendors.fmp.constituents import fetch_current_sp500
from core.data.vendors.fmp.intraday import (
    INTRADAY_INTERVALS,
    fetch_intraday_history,
    generate_intraday_chunks,
)
from core.data.vendors.fmp.storage import load_universe_symbols, safe_filename, write_atomic
...
def fetch_all(
    ...
    Args:
        ...
        interval: Key into :data:`core.data.vendors.fmp.intraday.INTRADAY_INTERVALS`.
