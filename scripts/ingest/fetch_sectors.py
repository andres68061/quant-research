...
Usage:
    ...
    python scripts/ingest/fetch_sectors.py --force  # Force refresh all
    python scripts/ingest/fetch_sectors.py --symbols AAPL MSFT  # Specific symbols
"""
...
from core.data.store.duckdb_io import read_parquet
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: python scripts/ingest/fetch_sectors.py",
    )
    parser.add_argument(
        "--prices",
        type=str,
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force refresh all symbols (even if not stale)"
    )
    parser.add_argument(
        "--symbols", nargs="+", help="Specific symbols to fetch (default: all from prices.parquet)"
    )
    ...
    if unknown_count > 0:
        ...
        unknown_symbols = updated_df[updated_df["sector"] == "Unknown"]["symbol"].tolist()
    ...
    print()
    print("Next steps:")
    print("  - Run quarterly: python scripts/ingest/update_sectors.py")
    print("  - Add new stock: python scripts/ingest/add_symbol.py TICKER")
