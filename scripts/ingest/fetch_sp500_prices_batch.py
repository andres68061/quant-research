...
Usage:
    python scripts/ingest/fetch_sp500_prices_batch.py --batch 50
    python scripts/ingest/fetch_sp500_prices_batch.py --all  # Fetch all missing
"""
...
from core.data.factors.prices import fetch_symbols_batch
from core.data.store.duckdb_io import read_parquet, write_parquet
...
def fetch_batch_symbols(
    symbols: List[str], existing_prices: pd.DataFrame
...
def categorize_failures(failed_symbols: List[str]) -> dict:
    ...
    return {"No data/Delisted": [(s, "No data returned from batch fetch") for s in failed_symbols]}
...
def main(batch_size: int = 50, start_index: int = 0, exclude_failed: bool = True):
    ...
    if start_index > 0:
        ...
        print(
            f"   Ignoring start_index={start_index}, fetching from beginning of current missing list"
        )
    ...
    print(
        f"Estimated time: ~{total_minutes:.1f} minutes (vs ~{len(to_fetch) * 2.5 / 60:.1f} min sequential)"
    )
    print()
    ...
    if still_missing:
        ...
        if previously_failed or failed:
            ...
            print("   Review data/sp500_failed_symbols.json for details.")
        print("\nNext steps:")
        print("  1. Run: python scripts/experiments/analyze_sp500_constituents.py")
...
Examples:
  ...
  python scripts/ingest/fetch_sp500_prices_batch.py --all
  
  # Retry previously failed symbols
  python scripts/ingest/fetch_sp500_prices_batch.py --batch 50 --include-failed
        """,
    )
    parser.add_argument(
        "--batch", type=int, default=50, help="Number of symbols per batch (default: 50)"
    )
    parser.add_argument(
        "--start",
        type=int,
    ...
    parser.add_argument(
        "--all", action="store_true", help="Fetch all missing symbols (overrides --batch)"
    )
    parser.add_argument(
        "--include-failed",
        action="store_true",
    ...
    main(batch_size=batch_size, start_index=args.start, exclude_failed=not args.include_failed)
