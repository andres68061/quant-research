...
#
# Usage:  nohup bash scripts/ingest/run_full_ingestion.sh "1 2 3 4" > runtime/logs/ingestion_all.out 2>&1 &
...
for wave in $WAVES; do
    ...
    "$PYTHON" scripts/ingest/ingest_fmp.py \
        --wave "$wave" \
        --rate "$RATE" \
        --workers "$WORKERS" \
        --log-file "runtime/logs/ingest_wave${wave}.log"
    status=$?
