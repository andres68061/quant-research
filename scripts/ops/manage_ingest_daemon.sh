...
#
# Usage:
#   bash scripts/ops/manage_ingest_daemon.sh install   # copy plist + load + start
#   bash scripts/ops/manage_ingest_daemon.sh status    # is it running, how far along
#   bash scripts/ops/manage_ingest_daemon.sh stop      # stop and unload
#   bash scripts/ops/manage_ingest_daemon.sh restart
#   bash scripts/ops/manage_ingest_daemon.sh logs      # tail the daemon log
...
case "${1:-status}" in
  ...
  status)
    ...
    launchctl print "$DOMAIN/$LABEL" 2>/dev/null \
      ...
      || echo "  not loaded (run: bash scripts/ops/manage_ingest_daemon.sh install)"
    echo
    echo "== heartbeat =="
