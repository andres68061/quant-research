...
instead — see ``scripts/manage_ingest_daemon.sh install``. This script is the
no-privileges fallback that covers session independence only.

Usage:
    python scripts/spawn_ingest_daemon.py
    python scripts/spawn_ingest_daemon.py --waves "2 3 4" --rate 600
"""
...
DAEMON = ROOT / "scripts" / "ingest_daemon.sh"
LOG = ROOT / "logs" / "ingest_daemon.log"
...
def main() -> int:
    ...
    if os.fork() > 0:
        ...
        print(f"logs: {LOG}")
        print("it is now independent of this session; check with scripts/ingest_status.py")
        return 0
